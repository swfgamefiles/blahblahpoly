...
var Engine = function() {
	...
	window.loadFullgame = function(string) {
		_gthis.fullgameVideo = JSON.parse(string);
	};
	window.clearFullgame = function(string) {
...
Engine.prototype = {
			...
			case 0:
				...
				if(_gthis.control.frame + 1 >= player.video.pauseFrame) {
					if(_gthis.fullgameVideo == null) {
						...
						console.log("src/Engine.hx:102:","[PAUSE ] @ " + (_gthis.control.frame + 1));
	...
	,primeControls: function(buffer) {
		...
		if(this.initialDirection == 1) {
			if(buffer) {
				console.log("src/Engine.hx:206:","---> Holding controls: LEFT.");
			}
			this.sendGameInput(37,true);
		}
		if(this.initialDirection == 2) {
			if(buffer) {
				console.log("src/Engine.hx:211:","---> Holding controls: RIGHT.");
			}
			this.sendGameInput(39,true);
		}
		if(this.initialDirection == 0) {
			if(buffer) {
				console.log("src/Engine.hx:216:","---> Holding controls: NONE.");
			}
		}
	}
	,resetLevel: function(slot,replay) {
		...
		console.log("src/Engine.hx:223:","[" + (replay ? "REPLAY" : "RESET to") + " " + (slot == null ? "start" : "slot " + (slot == null ? "null" : "" + slot) + "...") + "]");
	...
	,handleInterfaceInput: function(input,ctrlKey,altKey) {
		...
		if(input == CoffeeInput.StepFrame && this.control.paused) {
			console.log("src/Engine.hx:244:","[STEP  ] @ " + (this.control.frame + 1));
			this.triggerPausedCallback();
			return true;
		}
		if(input == CoffeeInput.Pause) {
			if(!oldControl.paused) {
				console.log("src/Engine.hx:252:","[PAUSE ] @ " + (this.control.frame + 1));
			}
			this.control.pause();
			return true;
		}
		...
		if(playAction) {
			...
			if(oldControl.paused) {
				console.log("src/Engine.hx:273:","[PLAY  ] @ " + this.control.frame);
			}
			this.triggerPausedCallback();
			return true;
		}
		...
		if(input._hx_index == 6) {
			...
			if(ctrlKey && !altKey) {
				...
				console.log("src/Engine.hx:307:","[SAVE slot " + slot + "] @ " + this.control.frame);
				console.log("src/Engine.hx:308:","data: " + video.toString());
	...
	,onScene: function(name) {
		console.log("src/Engine.hx:319:","[SCENE " + name + "]");
...
var CoffeeInput = $hxEnums["CoffeeInput"] = { __ename__:true,__constructs__:null
	...
	,Reset: {_hx_name:"Reset",_hx_index:5,__enum__:"CoffeeInput",toString:$estr}
	,Slot: ($_=function(code) { return {_hx_index:6,code:code,__enum__:"CoffeeInput",toString:$estr}; },$_._hx_name="Slot",$_.__params__ = ["code"],$_)
};
CoffeeInput.__constructs__ = [CoffeeInput.StepFrame,CoffeeInput.Pause,CoffeeInput.PlaySlow,CoffeeInput.PlayNormal,CoffeeInput.PlayFast,CoffeeInput.Reset,CoffeeInput.Slot];
...
Main.infoTrace = function(str) {
	console.log("src/Main.hx:5:","    " + str);
};
Main.main = function() {
	console.log("src/Main.hx:9:","  _____           _              _      _____       __  __          \n |_   _|         | |            | |    / ____|     / _|/ _|         \n   | |  _ __  ___| |_ __ _ _ __ | |_  | |     ___ | |_| |_ ___  ___ \n   | | | '_ \\/ __| __/ _` | '_ \\| __| | |    / _ \\|  _|  _/ _ \\/ _ \\\n  _| |_| | | \\__ \\ || (_| | | | | |_  | |___| (_) | | | ||  __/  __/\n |_____|_| |_|___/\\__\\__,_|_| |_|\\__|  \\_____\\___/|_| |_| \\___|\\___|");
	console.log("src/Main.hx:15:","Instant Coffee is enabled.");
...
VideoRecorder.prototype = {
	recordKey: function(frame,keyCode,down,silent) {
		...
		case 0:
			...
			if(!silent) {
				console.log("src/Video.hx:131:","---> " + Video.showActionCode(action) + " " + (down ? "down" : "up  ") + " @ " + frame);
			}
			break;
		case 1:
			return;
